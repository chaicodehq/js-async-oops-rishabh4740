...
export async function checkSeatAvailability(trainNumber, date, classType) {
  ...
  const randomSeats = Math.floor(Math.random() * 51);
  const waitlist = Math.floor(Math.random() * 21);
  
  const available = randomSeats > 0 && Math.random() < 0.7;
...
export async function bookTicket(passenger, trainNumber, date, classType) {
  if (!passenger || !passenger.name || !passenger.age || !passenger.gender) {
    throw new Error("Passenger details (name, age, gender) are incomplete!");
  }
  ...
  const currentFare = fares[classType];
...
export async function bookMultipleTickets(passengers, trainNumber, date, classType) {
  ...
  for (const person of passengers) {
    try {
      const booking = await bookTicket(person, trainNumber, date, classType);
      results.push(booking);
    } catch (error) {
      results.push({ passenger: person, error: error.message });
    }
  }
  return results;
}