...
export function assignRider(order) {
  return new Promise((resolve, reject) => {
    ...
    const selectedRider = RIDER_POOL[Math.floor(Math.random() * RIDER_POOL.length)];
...
export function processDelivery(restaurant, items) {
  return placeOrder(restaurant, items)
    .then(order => confirmOrder(order))
    .then(confirmedOrder => assignRider(confirmedOrder))
    .then(assignedOrder => deliverOrder(assignedOrder))
    .catch(error => ({ 
      error: error.message, 
      status: "failed" 
    }));
...
export function processMultipleOrders(orderList) {
  ...
  const orderPromises = orderList.map(item => 
    processDelivery(item.restaurant, item.items)
  );
