...
export class DabbaService {
  constructor(serviceName, area) {
    if (typeof serviceName !== 'string' || typeof area !== 'string') {
      throw new Error('Service name and area must be strings');
    }
    this.serviceName = serviceName;
    this.area = area;
    this.customers = [];
  ...
  addCustomer(name, address, mealPreference) {
    ...
    if (typeof name !== 'string' || typeof address !== 'string' || !validPreferences.includes(mealPreference)) {
      return null;
    }
