...
export class TempleQueue {
  ...
  static merge(queue1, queue2) {
    ...
    const newCapacity = queue1.toArray().length + queue2.toArray().length;
    
    const mergedQueue = new TempleQueue(newName, newCapacity);
    
    const allDevotees = [...queue1.toArray(), ...queue2.toArray()];
